...
async function zySignUp({ name, email, phone, password }) {
  if (ZY_DEMO || !sb) {
    // Simulate Supabase's "confirm your email" response.
...
async function zyResend(email) {
  ...
  const { error } = await sb.auth.resend({ type: 'signup', email, options: { emailRedirectTo: zyVerifyRedirect() } });
  if (error) throw error;
  return {};
}
