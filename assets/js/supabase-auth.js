...
async function zySignUp({ name, email, password }) {
  if (ZY_DEMO || !sb) {
    ...
    return { demo: true, needsVerification: true, email: email };
  }
  ...
  try { localStorage.setItem('zy_pending_email', email); } catch (e) {}
  return { needsVerification: !result.data.session, email: email, session: result.data.session };
}
...
async function zyVerifyFromUrl() {
  ...
  var hashAccess = hash.get('access_token');
  ...
  if (hashAccess) {
    ...
    return s.data.session ? { status: 'success' } : { status: 'error', message: 'No active session' };
  }
  if (!token_hash) return { status: 'pending' };

  var v = await sb.auth.verifyOtp({ token_hash: token_hash, type: type });
  if (v.error) {
    return { status: /expired/i.test(v.error.message) ? 'expired' : 'error', message: v.error.message };
  }
