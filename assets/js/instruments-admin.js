...
(function(){
  ...
  function render(){
    ...
    document.getElementById('inAlt').textContent=ALL.filter(function(x){return ['Private Equity','Collectibles','Derivatives'].indexOf(x.product)>-1;}).length;
    document.getElementById('inCash').textContent=ALL.filter(function(x){return ['Cash Funds','Cash on Hand'].indexOf(x.product)>-1;}).length;
  }
  ...
  document.getElementById('in-save').addEventListener('click', async function(){
    ...
    var res;
    if(inEditId){
      res = await sb.from('instruments').update(payload).eq('id',inEditId);
    } else {
      res = await sb.from('instruments').insert(payload);
    }
  ...
  document.querySelectorAll('.filter-bar .chip').forEach(function(c){
    c.addEventListener('click',function(){
      ...
      c.classList.add('active'); inFilter=c.dataset.prod||''; render();
    });
  });
