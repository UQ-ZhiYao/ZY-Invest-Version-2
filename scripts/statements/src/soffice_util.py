"""
Vendored from Claude Code's `xlsx` skill (scripts/office/soffice.py) — kept
as a plain copy so this tool has no dependency on that skill being present
wherever it eventually runs. On a normal Linux host `_needs_shim()` returns
False immediately and this is a thin wrapper around `subprocess.run(["soffice", ...])`.
